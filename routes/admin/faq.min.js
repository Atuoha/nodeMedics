...
router.post("/delete/:id",(b,d)=>{Faq.findOne({_id:b.params.id}).then(a=>{a.delete().then(c=>{b.flash("success_msg",`${c.question} has been deleted  successfully :)`);d.redirect("/admin/faq")}).catch(c=>console.log(c))}).catch(a=>console.log(a))});
router.post("/multiaction",(b,d)=>{console.log(b.body.checkboxes);Faq.find({_id:b.body.checkboxes}).then(a=>{a.forEach(c=>{c.delete().then(e=>{b.flash("success_msg","Faqs has been deleted  successfully :)");d.redirect("/admin/faq")}).catch(e=>console.log(e))})}).catch(a=>console.log(a))});module.exports=router;