...
router.get("/delete/:id",(a,c)=>{Testimony.findOne({_id:a.params.id}).then(b=>{b.delete().then(d=>{a.flash("success_msg",`${b.field} has been deleted successfully :)`);"Admin"===loggedUser.role?c.redirect("/admin/testimony"):c.redirect(`/admin/testimony/loggedUser/${a.user.id}`)}).catch(d=>console.log(d))}).catch(b=>console.log(b))});
router.post("/multiaction",(a,c)=>{console.log(a.body.checkboxes);Testimony.find({_id:a.body.checkboxes}).then(b=>{b.forEach(d=>{d.delete().then(e=>{a.flash("success_msg","Testimonies has been deleted  successfully :)");"Admin"===loggedUser.role?c.redirect("/admin/testimony"):c.redirect(`/admin/testimony/loggedUser/${a.user.id}`)}).catch(e=>console.log(e))})}).catch(b=>console.log(b))});module.exports=router;