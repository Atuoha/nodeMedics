...
router.post("/create",(b,d)=>{let a="";if(!isEmpty(b.files)){var c=b.files.file;a=Date.now()+"-"+c.name;c.mv("./public/uploads/"+a,e=>{e&&console.log(e)})}c=new Research;c.name=b.body.name;c.location=b.body.location;c.estDate=b.body.estDate;c.intro=b.body.intro;c.content=b.body.content;c.tag=b.body.tag;c.file=a;c.save().then(e=>{b.flash("success_msg","Research registered successfully :)");d.redirect("/admin/labs")}).catch(e=>console.log(e))});
router.post("/dummy",(b,d)=>{for(let a=0;a<b.body.number;a++){const c=new Research;c.name=faker.name.title();c.location=faker.random.word();c.content=faker.lorem.sentence();c.intro=faker.lorem.sentence();c.tag=faker.random.word();c.file="img_place.png";c.estDate=new Date;c.save().then(e=>{b.flash("success_msg",`${b.body.number} Dummy Labs registered successfully :)`);d.redirect("/admin/labs")}).catch(e=>console.log(e))}});
...
router.post("/update/:id",(b,d)=>{Research.findOne({_id:b.params.id}).then(a=>{let c=a.file;if(!isEmpty(b.files)){let e=b.files.file;c=Date.now()+"-"+e.name;e.mv("./public/uploads/"+c,f=>{f&&console.log(f)});"img_place.png"!==a.file&&fs.unlink("./public/uploads/"+a.file,f=>{f&&console.log(f)})}a.name=b.body.name;a.tag=b.body.tag;a.location=b.body.location;a.estDate=b.body.estDate;a.intro=b.body.intro;a.content=b.body.content;a.file=c;a.save().then(e=>{b.flash("success_msg",`${a.name} has been updated successfully :)`);
